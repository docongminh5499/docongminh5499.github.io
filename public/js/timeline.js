...
(function($, window, document, undefined) {
  ...
  Timeline.prototype.init = function() {
    ...
    if (contentList.length == 0)
      this.$element
        .css('opacity', 1)
        .html('<h3>There are no events at this point in time.<br><br>Please add some content.</h3>');
      ...
      $.proxy(function() {
        ...
        function addIdsAndClasses(self) {
          ...
          if (timelineComponents['eventsContentList'].length == 1) {
            ...
            timelineComponents['eventsContentList']
              .addClass(self.settings.animation_baseClass)
              .first()
              .attr('id', 'first')
              .end()
              .last()
        ...
        // If no class found at all...
          ...
          if (this.settings.dateOrder == 'normal') {
            ...
            timelineComponents['eventsContent'].find('li').filter(
              $.proxy(function(index, element) {
                var data = this._timelineData($(element), 'date');
  ...
  Timeline.prototype._create = function() {
    var $timelineWrapper = '<div class="timeline"></div>',
      ...
      $nextButton =
        '<a href="" class="' + this.settings.iconBaseClass + ' ' + this.settings.next_iconClass + ' next"></a>',
      $scrollRightButton =
        '<a href="" class="' +
        this.settings.iconBaseClass +
        ' ' +
      ...
      $pauseButton =
        '<a href="" class="' + this.settings.iconBaseClass + ' ' + this.settings.pause_iconClass + ' pause"></a>';
    ...
    if (this.settings.useNavBtns == true || this.settings.useScrollBtns == true) {
      ...
      if (this.settings.useNavBtns == false && this.settings.useScrollBtns == true)
        ...
        $timeline
          .find('#leftNav')
          .append($scrollLeftButton)
          .end()
          .find('#rightNav')
          .append($scrollRightButton);
          ...
          if (typeof customDisplay !== 'undefined') {
            ...
            if (insertMethod == 'append')
              $eventDateDisplay.append(
                dateLink + dayPart + numSuffix(dayPart) + '<br>' + getMonthName(monthPart) + '</a>'
              );
            // For use with the addEvent method... creates new timeline events and places them where specified.
            ...
            if (insertMethod == 'append')
              $eventDateDisplay.append(dateLink + getMonthName(monthPart) + '<br>' + yearPart + '</a>');
            // For use with the addEvent method... creates new timeline events and places them where specified.
            else if (insertMethod == 'after')
              $arrangementEvent.after(dateLink + getMonthName(monthPart) + '<br>' + yearPart + '</a>');
    ...
    if (this.settings.autoplay == true) {
      ...
      this.$element
        ...
        .on(
          ...
          $.proxy(this._setup.autoplay.changeButtons, this)
        )
      ...
      function autoplay(self) {
        ...
        function interval() {
          ...
          if (isPaused === false && this._elementInViewport(this.element)) {
            ...
            if (percentTime >= 100) {
              ...
              if (
                (this.settings.dateOrder == 'normal' && current == this._setup.autoplay.countEvents()) ||
        ...
        function changeButtons(event) {
          ...
          var data = event.data,
            ...
            $playButton =
              '<a href="" class="' + this.settings.iconBaseClass + ' ' + this.settings.play_iconClass + ' play"></a>';
    ...
    if (goToTimelineLink.length > 0) {
      ...
      function gotoTimeline(event) {
        ...
        // If not, then use a smooth scroll and then execute the public method interally afterwards.
          ...
          instanceRef.goTo(
            date,
            { smoothScroll: true, speed: scrollSpeed, offset: scrollOffset, easing: scrollEasing },
            instanceRef
          );
  ...
  Timeline.prototype.addEvent = function(html, insertMethod, arrangementDate) {
    ...
    var newDate = html.split('date')[1].split('"')[2],
      ...
      $eventContent = timelineComponents['eventsContent'].find('li').filter(
        $.proxy(function(index, element) {
          var data = this._timelineData($(element), 'date');
  ...
  Timeline.prototype.goTo = function(
    date,
  ...
  Timeline.prototype._updateSlide = function(timelineComponents, timelineTotalWidth, string) {
    ...
    if (string == 'right')
      this._translateTimeline(timelineComponents, translateValue - wrapperWidth, wrapperWidth - timelineTotalWidth);
  ...
  Timeline.prototype._updateVisibleContent = function(event, eventsContent) {
    ...
    (visibleContent = eventsContent.find('.selected')),
      ...
      (newContent = eventsContent.find('li').filter(
        $.proxy(function(index, element) {
          var data = this._timelineData($(element), 'date');
